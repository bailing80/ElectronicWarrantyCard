...
public class CardHospitalServiceImpl  implements CardHospitalService {
	...
	public int save(CardHospitalDetails record)
	{
		...
		{
			CardHospital temp = cardHospitalMapper.findHospital(record.getName(),record.getTelephone());
			if(temp == null )
			{
	   ...
	   try {
	    ...
	    // 创建Excel工作薄
           ...
           for (int i = sheet.getFirstRowNum(); i < sheet.getLastRowNum() + 1; i++) 
           {
               ...
               {
            	    CardHospital temp=cardHospitalMapper.findHospital(hospital.getName(),hospital.getTelephone());   
            	    if(temp == null )
            	    {
            	    	hospital.setCreatetime(DateTimeUtils.getDateTime());
            	    	hospital.setCustomercode(getLatestCustomercode());
