...
import com.louis.kitty.admin.util.HttpClientUtil;
...
public class CardPersonalServiceImpl  implements CardPersonalService {
  ...
  //用户名
    ...
    private static String smsText="！请在注册电子保修卡页面输入以完成注册。5分钟内有效。";
	@Override
...
/*		
   ...
   private Object getCellValue(Cell cell) 
   {
	...
	@Override
	 public String getDetailsById(String serialnumber) {//查看详情
	   CardPersonal personal=cardPersonalMapper.selectAndserialnumber(serialnumber);
       if(null==personal||!serialnumber.equals(personal.getSerialnumber())) {
    	   return "未查询到相关报修信息";
       }else {
       ...
       Date date = new Date();
       Date date1 = new Date();
       String datebailing = null;
       String datebailing2=null;
       try {
    	   String bailing2=personal.getPurchasedate();
    	   Calendar calendar = new GregorianCalendar();
           date1 = sdFormat.parse(personal.getPurchasedate());
           calendar.setTime(date1); 
           calendar.add(calendar.YEAR, 2);//把日期往后增加一年.整数往后推,负数往前移动
           calendar.add(Calendar.DAY_OF_MONTH, -1);
           date = calendar.getTime();
           String bailing = sdFormat.format(date);
           String now = new SimpleDateFormat("yyyy年MM月dd").format(date);
           String now2 = new SimpleDateFormat("yyyy年MM月dd").format(date1);
           datebailing=now;
           datebailing2=now2;
       ...
       String strboy=personal.getSex();
       String strwomen=personal.getSex();
       String boyandwomen="";
       if(!"女".equals(personal.getSex()))
       {
    	   boyandwomen="先生";
       }
       else {
         ...
         String str=name+boyandwomen+ ",您的呼吸机于"+datebailing2+"日购买,保修期至"+datebailing+"日。";
	    return str;
	 }
...
//	}
	 ...
	 */
	@Override
	public String Smsmessage(String sphone,String mseid)throws Exception
	{
		String message="";
	    HttpClientUtil client = HttpClientUtil.getInstance();		    
		int  result = client.sendMsgUtf8(Uid, Key, "验证码为:"+mseid+smsText, sphone);
	   // int  bl =1;
		if(result < 0)
		{
			message=client.getErrorMsg(result);	
		}else
		{
